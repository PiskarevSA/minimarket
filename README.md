...
Первый выпускной проект на курсе go-advanced от Яндекс Практикум
...
5. `sqlc.yaml` - конфигурационный файл для sqlc.
...
* `.golangci.yaml` - конфигурационный файл для линтера golangci-lint
* `.mockery.yaml` - конфигурационный файл для генератора mockery.
* `.sqlfluff` - конфигурационный файл для линтера sqlfluff
* `oapi-codegen.yaml` - конфигурационный файл для генератора oapi-codegen.
* `Taskfile.yaml` - конфигурационный файл для утилиты
[task](https://taskfile.dev/) (аналог make).

## Разработка

Требует установки task

```sh
  # Сгенерировать моки (требует установки в систему mockery)
  task generate:mockery:all
  # Сгенерировать oapi (требует установки в систему oapi-codegen)
  task generate:oapi:all
  # Сгенерировать sqlc (требует установки в систему sqlc)
  task generate:sqlc:all
  # Линтинг golangci-lint (запускается из docker)
  task lint:golangci-lint:all
  # Линтинг sqlfluff (запускается из docker)
  task lint:sqlfluff:all
```

## Сборка

Не требует генерации, так как её результат уже добавлен в репозиторий
...
* Пискарёв Семён <piskarevsa@mail.ru>
* Чуйков Михайл <mtchuikov@yandex.ru>

## Лицензия

Copyright © 2025, Minimarket OJSC
